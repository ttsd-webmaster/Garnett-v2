...
usersRef.once('value', (snapshot) => {
  snapshot.forEach((user) => {
    ...
    if (user.val().hasOwnProperty("registrationToken") && !user.val().hasOwnProperty("class")) {
      // console.log("skipped");
    }
    else if (user.val().status !== 'alumni' && user.val().status !== 'pledge') {
      ...
      if (user.val().year === '4th Year' || user.val().year === '5th Year' || user.val().year === '5th year') {
