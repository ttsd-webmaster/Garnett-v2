...
exports.create_url = function (req, res) {
  const { urlToShorten, newUrl, displayName } = req.body;
  const payload = { shortUrl: newUrl, origUrl: urlToShorten, createdBy: displayName }
...
exports.update_url = function (req, res) {
  const { urlToUpdate, newUrl, displayName } = req.body;
  const payload = { shortUrl: newUrl, origUrl: urlToUpdate, createdBy: displayName }
  const urlRef = admin.database().ref('/urls');
  urlRef.orderByChild('shortUrl').equalTo(newUrl).once('value', (urls) => {
    if (urls.exists()) {
      urls.forEach((url) => {
